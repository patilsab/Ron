...
# ============================================================
# FIX PICOM "BLANK UNTIL INTERACTION" BUG
# ============================================================
#
# Symptom: a newly opened Terminator window (or any window) is
# not painted after a mouse click, but appears as soon as a key
# is pressed inside it (e.g. Enter). This is a well known picom
# compositing bug caused by "unredir-if-possible", which lets
# picom bypass compositing for what it thinks is a fullscreen
# window. In a tiling WM like i3 this heuristic misfires and
# leaves newly mapped windows unpainted until a damage event
# (like a keypress) forces a repaint.
#
# Fix: force "unredir-if-possible" off and make sure damage
# tracking is enabled, so every new window is composited and
# painted immediately when it is mapped/clicked.

if [[ -f "$PICOM_CONFIG" ]]; then

    info "Patching Picom configuration to fix window render bug..."

    # Disable unredir-if-possible, however it is currently set
    # (true/false/commented), and set it to false explicitly.
    if grep -q '^[[:space:]]*#\?[[:space:]]*unredir-if-possible' "$PICOM_CONFIG"; then
        sed -i \
            's/^[[:space:]]*#\?[[:space:]]*unredir-if-possible[[:space:]]*=.*/unredir-if-possible = false;/' \
            "$PICOM_CONFIG"
    else
        echo "unredir-if-possible = false;" >> "$PICOM_CONFIG"
    fi

    # Make sure damage tracking is on, so mapped/clicked windows
    # are always repainted instead of relying on a later event.
    if grep -q '^[[:space:]]*#\?[[:space:]]*use-damage' "$PICOM_CONFIG"; then
        sed -i \
            's/^[[:space:]]*#\?[[:space:]]*use-damage[[:space:]]*=.*/use-damage = true;/' \
            "$PICOM_CONFIG"
    else
        echo "use-damage = true;" >> "$PICOM_CONFIG"
    fi

    success "Picom render fix applied (unredir-if-possible = false)."

fi

# Make sure i3 doesn't launch a second, conflicting picom
# instance (e.g. once from i3 config and once from
# polybar_launch.sh), which can also cause windows to render
# inconsistently. --replace makes any relaunch take over
# cleanly instead of stacking compositor instances.
if grep -q 'exec_always --no-startup-id picom' "$I3_CONFIG"; then

    sed -i \
        's/exec_always --no-startup-id picom\([^\n]*\)/exec_always --no-startup-id picom --replace\1/' \
        "$I3_CONFIG"

    # Avoid accidentally doubling up --replace if the script is run again.
    sed -i \
        's/picom --replace --replace/picom --replace/' \
        "$I3_CONFIG"

    success "Picom autostart made idempotent (--replace)."
